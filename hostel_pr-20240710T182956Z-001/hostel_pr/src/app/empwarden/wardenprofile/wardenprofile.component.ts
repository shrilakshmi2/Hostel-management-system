...
@Component({
  selector: 'app-wardenprofile',
  templateUrl: './wardenprofile.component.html',
  styleUrls: ['./wardenprofile.component.css']
})
export class WardenprofileComponent {
  @Input() obj
  profile={fetched:false,pic:'',name:'',mobile:'',email:'',adhar:'',current_state:'',logs:[],adress:{}}
  ...
  async loadresources(){
    ...
    this.profile.mobile=result.data.Mobile_No
    this.profile.email=result.data.E_mail
    this.profile.adhar=result.data.Aadhar
    ...
    if(result.data.others.hasOwnProperty('logs')){
      ...
      for(let i in this.profile.logs){
        let date=new Date(this.profile.logs[i].time)
  ...
  async change_detail_send(){
    ...
    form.append('mode','emp')
    form.append('id2',this.profile.mobile)
  ...
  async changepass_final(pass1,pass2,ev,mail){
    ...
    body:JSON.stringify({id:this.obj.id,unq:this.obj.unq,mail:mail.innerHTML,pass:pass1,mode:'emp'})})
    let result=await res.json()
    if(result.status=='unable'){
  ...
  Seach_logs(val){
    ...
    for(let i of this.logsserach.cur_logs){
      ...
      if(this.logsserach.mode=='3'){
        if(i.reason.toLowerCase().includes(val.toLowerCase()))
          arr.push(i)
      }
      else{
        let temp=val.split('-')
